...
const BlogAdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editId) {
        // EDITING EXISTING BLOG - Use patchBlog with ID and data
        response = await authAPI.patchBlog(editId, blogData);
        
        if (response.success || response.data || response) {
          // Update the existing blog in the list
          const updatedBlog = response.data || response;
          setBlogs(prev => 
            prev.map(blog => 
              blog._id === editId 
                ? { ...blog, ...updatedBlog, _id: editId } // Ensure ID is preserved
                : blog
            )
          );
        ...
        // CREATING NEW BLOG
        response = await authAPI.postBlog(blogData);
      ...
      alert(`Error ${editId ? 'updating' : 'creating'} blog. Please try again.`);
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              onClick={() => {
                if (showForm && editId) {
                  // If we're editing and want to hide form, reset everything
                  resetForm();
                } else {
...
//   // Helper function to convert HTML description to array of strings
//   const convertDescriptionToArray = (htmlDescription) => {
...
//     // Get text content and split by common delimiters
...
//     // Split by paragraphs, line breaks, or sentences
//     let paragraphs = textContent
//       .split(/\n\s*\n|\r\n\r\n/) // Split by double line breaks (paragraphs)
//       .map(p => p.trim())
//       .filter(p => p.length > 0);

//     // If no paragraph breaks found, split by single line breaks
...
//     // If still only one item, split by sentences
//     if (paragraphs.length <= 1 && textContent.length > 200) {
//       paragraphs = textContent
//         .split(/[.!?]+\s+/)
//         .map(s => s.trim())
//         .filter(s => s.length > 0)
...
//     return paragraphs.length > 0 ? paragraphs : [textContent || 'Default content'];
//   };

//   // Helper function to convert array back to HTML for editing
//   const convertArrayToHTML = (descriptionArray) => {
...
//   // FIXED handleSubmit function with proper description conversion
//   const handleSubmit = async (e) => {
//     e.preventDefault();
...
//       // Convert HTML description to array format for API
//       const descriptionArray = convertDescriptionToArray(formData.description);
      
//       const blogData = {
//         ...formData,
//         description: descriptionArray, // Convert to array format
//         slug: formData.slug || generateSlug(formData.title)
//       };
...
//   // Handle edit button click - convert array back to HTML for editing
//   const handleEditClick = (blog) => {
//     setFormData({
//       title: blog.title,
//       subTitle: blog.subTitle,
//       subject: blog.subject,
//       description: convertArrayToHTML(blog.description), // Convert array back to HTML
//       slug: blog.slug,
//       pictures: blog.pictures || [],
//     });
//     setEditId(blog._id);
...
//                             ? blog.description.slice(0, 2).join(' ') 
